...
def get_digit_codes(digit_codes):
    ...
    for (digit, index) in [(1, 0), (7, 1), (4, 2), (8, 9)]:
        digits_to_codes[digit] = sort_string(digit_codes[index])
    for code in [digit_codes[x] for x in range(3, 9)]:
        digits_to_codes[get_digit_by_comparing_to_1_and_4(setify(code), digits_to_codes[1], digits_to_codes[4])] = sort_string(code)
    return digits_to_codes


def translate_output(output_codes, digits_to_codes):
    codes_to_digits = {val: idx for idx, val in enumerate(digits_to_codes)}
    output_digits = ''
    for code in output_codes:
        output_digits += str(codes_to_digits[sort_string(code)])
